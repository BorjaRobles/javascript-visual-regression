...
exports.config = {
  ...
  before: function (capabilities, specs) {
    ...
    browser.addCommand("login", function(path, user, pw) {
      this.url(url + encodeURIComponent(path))
        .setValue("input[placeholder='Email or Username']", user)
        .setValue('#password', pw)
        .submitForm('#submit');
    });

    // Script for fake time
    browser.addCommand('fakeTime', function (timestamp) {
      const LOLEX_LIB = fs.readFileSync('node_modules/lolex/lolex.js', 'utf8');

      browser.timeouts('script', 500);
      return browser.executeAsync((text, ts, done) => {
          const script = document.createElement('script');
      script.text = text;
      document.getElementsByTagName('head')[0].appendChild(script);

      window.lolex.install(ts, ['Date']);
      done();
    }, LOLEX_LIB.toString(), timestamp);
  });
},
